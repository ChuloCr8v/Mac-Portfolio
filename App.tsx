...
const INITIAL_WINDOWS: Record<AppID, WindowState> = {
  finder: { id: 'finder', title: 'Finder', isOpen: true, isMinimized: false, isMaximized: false, zIndex: 55, position: { x: 100, y: 80 }, size: { width: 800, height: 500 } },
  terminal: { id: 'terminal', title: 'Terminal - zsh', isOpen: false, isMinimized: false, isMaximized: false, zIndex: 54, position: { x: 200, y: 150 }, size: { width: 600, height: 400 } },
  mail: { id: 'mail', title: 'Mail', isOpen: false, isMinimized: false, isMaximized: false, zIndex: 53, position: { x: 150, y: 100 }, size: { width: 700, height: 500 } },
  vscode: { id: 'vscode', title: 'Visual Studio Code', isOpen: false, isMinimized: false, isMaximized: false, zIndex: 52, position: { x: 50, y: 50 }, size: { width: 900, height: 600 } },
  safari: { id: 'safari', title: 'Safari', isOpen: false, isMinimized: false, isMaximized: false, zIndex: 51 },
};
...
export const App = () => {
  ...
  const handleAppClick = (id: AppID) => {
    ...
    setWindows((prev) => {
      ...
      setActiveAppId(id);
      return { ...prev, [id]: { ...win, isOpen: true, zIndex: newZ, isMinimized: false } };
    });
  };
  ...
  const handleMaximize = (id: string) => {
    setWindows(prev => ({
      ...prev,
      [id]: { ...prev[id as AppID], isMaximized: !prev[id as AppID].isMaximized }
    }));
  };
  ...
  const handleCloseAllRequest = () => {
    ...
    const anyOpen = (Object.values(windows) as WindowState[]).some(w => w.isOpen);
    if (anyOpen) {
  ...
  const renderAppContent = (id: AppID) => {
    switch (id) {
      ...
      case 'vscode': return <VSCode theme={theme} />;
      default: return null;
    }
  };

  const DesktopIcon = ({ icon, label, onClick, className = "" }: { icon: React.ReactNode, label: string, onClick: () => void, className?: string }) => (
    <div 
        onClick={onClick}
        className="flex flex-col items-center justify-center gap-1.5 cursor-pointer group w-[88px]"
    >
        <div className={`p-3 rounded-2xl bg-white/20 dark:bg-black/20 hover:bg-white/30 dark:hover:bg-black/30 backdrop-blur-md border border-white/10 shadow-sm transition-all duration-200 group-hover:-translate-y-1 group-hover:shadow-md ${className}`}>
  ...
  const isAnyWindowMaximized = (Object.values(windows) as WindowState[]).some(w => w.isOpen && w.isMaximized && !w.isMinimized);
    ...
    <div className={`${theme}`}>
      <div className="w-screen h-screen overflow-hidden relative select-none bg-black">
        ...
        {WALLPAPERS.map((src, index) => (
            <div 
                key={src}
                className={`absolute inset-0 bg-cover bg-center transition-opacity duration-1000 ease-in-out ${index === wallpaperIndex ? 'opacity-100 z-0' : 'opacity-0 -z-10'}`}
                style={{ backgroundImage: `url('${src}')` }}
            />
        ))}

        <MenuBar 
          activeAppTitle={activeWindow?.isOpen && !activeWindow.isMinimized ? activeWindow.title : 'Finder'} 
          theme={theme}
        ...
        <div className="absolute top-24 left-6 flex flex-col space-y-6 items-start z-10">
          <DesktopIcon 
              icon={<div className="w-12 h-14 bg-white rounded-sm relative flex items-center justify-center shadow-lg border border-gray-300">
                  ...
                  <div className="absolute bottom-1.5 text-[7px] font-bold text-gray-400">PDF</div>
              </div>}
              label="Resume.pdf" 
          ...
          <DesktopIcon 
              icon={<div className="bg-[#0077b5] text-white p-2 rounded-xl shadow-lg w-12 h-12 flex items-center justify-center"><Linkedin size={28} /></div>}
              label="LinkedIn" 
              onClick={() => window.open(`https://linkedin.com/in/nkematu-bonaventure`, '_blank')}
          />

          <DesktopIcon 
              icon={<div className="bg-black text-white p-2 rounded-xl shadow-lg border border-white/20 w-12 h-12 flex items-center justify-center"><Github size={28} /></div>}
              label="GitHub" 
              onClick={() => window.open(`https://${RESUME.personal.github}`, '_blank')}
          />

          <DesktopIcon 
              icon={<div className="bg-black text-white p-2 rounded-xl shadow-lg border border-white/20 w-12 h-12 flex items-center justify-center"><XIcon size={24} /></div>}
              label="X" 
        ...
        {showSafariConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 backdrop-blur-sm">
              <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-200 dark:border-white/20 shadow-2xl rounded-xl p-6 w-80 md:w-96 transform scale-100 transition-all text-center animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Leaving Portfolio OS</h3>
                  <p className="text-gray-600 dark:text-gray-300 text-sm mb-6 leading-relaxed">
                      You are about to open an external link to <strong>{RESUME.personal.portfolio}</strong>. This will open in a new browser tab.
                  </p>
        ...
        {showCloseAllConfirm && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/20 backdrop-blur-sm">
              <div className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-200 dark:border-white/20 shadow-2xl rounded-xl p-6 w-80 md:w-96 transform scale-100 transition-all text-center animate-in fade-in zoom-in-95 duration-200">
                  ...
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">Close All Windows?</h3>
                  <p className="text-gray-600 dark:text-gray-300 text-sm mb-6 leading-relaxed">
                      Are you sure you want to close all currently open applications? Unsaved work in drafts might be lost.
                  </p>
                  <div className="flex space-x-3 justify-center">
                      <button 
        ...
        <Dock 
          ...
          openApps={(Object.values(windows) as WindowState[]).filter(w => w.isOpen).map(w => w.id)} 
